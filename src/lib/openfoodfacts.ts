...
//
// Note: unlike the React Native original we must NOT send a User-Agent header —
// browsers forbid setting it, and doing so trips the CORS preflight.
...
function normalizeProduct(product: OffProduct | undefined): FoodItem | null {
  ...
  return {
    ...
    proteinPerServing: round(pickNutrient(nutriments, "proteins", servingGrams)),
...
export async function searchProducts(
  query: string,
  { pageSize = 25, signal }: { pageSize?: number; signal?: AbortSignal } = {},
): Promise<FoodItem[]> {
  const trimmed = query.trim();
  if (!trimmed) return [];

  // A bare digit string is a barcode, not a name — look it up directly.
  if (/^\d{8,14}$/.test(trimmed)) {
    const item = await getProductByBarcode(trimmed, signal);
    return item ? [item] : [];
  }
